...
function ensure_capacity(c, v) {
  ...
  var data = Caml_array.make(cap | 0, 0);
...
function shrink_to_fit(v) {
  ...
  var data = Caml_array.make(v.length, 0);
  $$Array.blit(v.data, 0, data, 0, v.length);
  v.data = data;
  
}

function push(val$p, v) {
  ensure_capacity(v.length + 1 | 0, v);
  var length = v.length;
  v.length = length + 1 | 0;
  return Caml_array.set(v.data, length, val$p);
}
...
function pop(v) {
  ...
  var val$p = Caml_array.get(v.data, v.length - 1 | 0);
  Caml_array.set(v.data, v.length - 1 | 0, 0);
  v.length = v.length - 1 | 0;
  return Caml_option.some(val$p);
}
...
function apply(f, v) {
  return map2((function (prim0, prim1) {
...
function iota(start, end$p) {
  var l = Stdlib.abs(end$p - start | 0) + 1 | 0;
