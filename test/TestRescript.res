...
let assert_ = (a: 'a, b: int) => Js.log2(a, b)
let assert_string = (a: string, b: bool) => Js.log2(a, b)
let assert_bool = (a: bool, b: bool) => Js.log2(a, b)
let assert_list = (a: list<'a>, b: list<'a>) => Js.log2(a, b)
let assert_unit = (_, a: unit => unit) => Js.log2("assert_unit", a)
let assert_option = (a: option<'a>, b: option<int>) => Js.log2(a, b)
let assert_array = (a: array<'a>, b: array<int>) => Js.log2(a, b)
// let assert_raises = (~msg:string, exn,b:bool, unit =>(Vec.t('a, [> `R ]), int))=>(Vec.t('a, [> `R ]), int) => 'a => {Js.log2(msg, b)};
let assert_int_bool = (a: int, b: int) => Js.log2(a, b)
let make = _ => {
  let v = Vec.make()
...
let map = _ => {
  ...
  assert_list(list{1, 2, 3, 4, 5}, Vec.to_list(v'))

  Vec.map_in_place(succ, v')
  assert_list(list{2, 3, 4, 5, 6}, Vec.to_list(v'))

  let v'' = Vec.mapi((i, _) => i, v')
  assert_list(list{0, 1, 2, 3, 4}, Vec.to_list(v''))
}

let iter = _ => {
  let v = Vec.iota(0, 5)
  let expected = List.fold_left(\"+", 0, list{0, 1, 2, 3, 4, 5})
  let actual = ref(0)

  Vec.iter(i => actual := actual.contents + i, v)
  assert_(expected, actual.contents)

  let actual = ref(true)
  Vec.iteri((i, a) => actual := actual.contents && i == a, v)
  assert_string("iteri", actual.contents)
}

let cartesian_product = _ => {
  let a = Vec.of_list(list{1, 2, 3})
  let b = Vec.of_list(list{10, 20, 30})
  let expected = list{
    (1, 10),
    (1, 20),
    (1, 30),
...
let monad_ops = _ => {
  ...
  let expected = list{1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4}
  let f = i => Vec.of_list(list{i, i, i})
  let a = Vec.iota(1, 4)
...
let filter = _ => {
  let v = Vec.iota(0, 10)
  let even = i => mod(i, 2) == 0
  assert_list(list{0, 2, 4, 6, 8, 10}, \"@@"(Vec.to_list, Vec.filter(even, v)))
...
let exists = _ => {
  let v = Vec.of_list(list{1, 2, 3, 4, 5})
  assert_string("exists", Vec.exists(\"="(4), v))
  assert_string("not exists", \"@@"(not, Vec.exists(\"="(6), v)))
  assert_string(
    "exists empty",
    \"@@"(not, \"@@"(Vec.exists(_ => true), Vec.make())),
  )
}
...
let for_all = _ => {
  ...
  assert_string("not for_all", \"@@"(not, Vec.for_all(\"<="(3), v)))
...
let mem = _ => {
  ...
  assert_string("mem not", \"@@"(not, Vec.mem(101, v)))

  let a = [1, 2]
  let b = [1, 2]
  let v = Vec.of_list(list{[1, 2], a, [1, 2]})
  assert_string("memq", Vec.memq(a, v))
  assert_string("memq", \"@@"(not, Vec.memq(b, v)))
}
...
let zip = _ => {
  ...
  let expected = list{(1, 4), (2, 5), (3, 6)}
  assert_list(expected, \"@@"(Vec.to_list, Vec.zip(a, b)))

  let expected = list{5, 7, 9}
...
let equal = _ => {
  ...
  let a = Vec.of_list(list{1, 2, 3, 4, 5})
  let b = Vec.iota(1, 5)
  ...
  Vec.push(6, a)
  assert_string("equal diff length", \"@@"(not, Vec.equal(a, b)))
}

let compare = _ => {
  \"@@"(assert_(0), Vec.compare(Vec.make(), Vec.make()))

  let a = Vec.of_list(list{'a', 'b', 'c'})
  let b = Vec.of_list(list{'a', 'b', 'd'})
  ...
  let a = Vec.of_list(list{'a', 'b', 'c'})
  let b = Vec.of_list(list{'a', 'b', 'c', 'd', 'e'})
  assert_(-1, Vec.compare(a, b))

  let a = Vec.of_list(list{'a', 'b', 'c', 'd'})
  let b = Vec.of_list(list{'e', 'f'})
  assert_(-1, Vec.compare(a, b))
}
let assert_: (string, string, unit) => unit = (a: string, b: string) =>
  Js.log2(a == b)
let pretty_print2 = _ => {
  ...
  assert_("[abc; def]", \"@@"(pp, Vec.of_list(list{"abc", "def"})), ())

  let pp = Vec.pretty_print(s => \"@@"(Int.to_string, String.length(s)))
  assert_("[3; 4; 5]", \"@@"(pp, Vec.of_list(list{"aaa", "abcd", "abcde"})))
}
let pretty_print = _ => {
  let pp = Vec.pretty_print(Int.to_string)
  ...
  let pp = Vec.pretty_print(s => s)
  Js.log("[abc; def]" == \"@@"(pp, Vec.of_list(list{"abc", "def"})))

  let pp = Vec.pretty_print(s => \"@@"(Int.to_string, String.length(s)))
  Js.log("[3; 4; 5]" == \"@@"(pp, Vec.of_list(list{"aaa", "abcd", "abcde"})))
}
